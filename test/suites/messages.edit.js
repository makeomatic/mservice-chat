...
describe('messages.edit', function testSuite() {
  ...
  before('create admin message', () => create(chat.services.message, ['foo'], { roomId: this.room.id, user: this.admin })
    .spread((message) => {
      this.message = message;
      this.messageId = message.id.toString();
    }));

  before('create user message', () => create(chat.services.message, ['bar'], { roomId: this.room.id, user: this.user })
    .spread((message) => {
  ...
  it('should returns error if invalid `id`', () => request(uri, {
    token: this.adminToken, id: 0, roomId: this.roomId, text: 'foo',
  })
  ...
  it('should returns error if message not found', () => request(uri, {
    token: this.adminToken, id: '0', roomId: this.roomId, text: 'foo',
  })
    .then(({ statusCode, body }) => {
  ...
  it('should returns error user is not message owner', () => request(uri, {
    token: this.userToken, id: this.messageId, roomId: this.roomId, text: 'foo',
  })
    .then(({ statusCode, body }) => {
